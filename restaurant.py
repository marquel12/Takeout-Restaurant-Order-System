def place_order(menu):
    ...
    place_order = True # this is setting the condition to true so the loop will run
    # TODO: Create a continuous while loop so customers can order multiple items
    while place_order: # this is stating that the loop will continue until the condition is false
        ...
        for food_category, options in menu.items():
         ...
         # TODO: Loop through the options for each food category, extracting the
            ...
            for meal, price in options.items():
                ...
                index += 1 # this is incrementing the index by 1
        ...
        if keep_ordering == "n" or keep_ordering == "N":
            # If the customer types 'n' or 'N', set the condition to False
            place_order = False
...
def update_order(order, menu_selection, menu_items):
    ...
    if menu_selection.isdigit():
        ...
        print("Invalid input. Please enter a menu number.")
        return order
